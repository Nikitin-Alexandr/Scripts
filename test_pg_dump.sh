...
do
        for (( i=1; i <= $num_iterations; i++ ))
                ...
                /root/clear.sh
                st=`date +"%s%3N"`
                pg_dump -Fc -Z$j -d $db_name -U postgres -f $path_to_backup/$db_name.dmp
                s2=$((`date +"%s%3N"` - $st))
                s3=`du -sh $path_to_backup|awk '{print $1}'`
...
do
        for (( i=1; i <= $num_iterations; i++ ))
                do
                s1="custom dump pigz="$j
                echo $s1"-"$i
                /root/clear.sh
                st=`date +"%s%3N"`
                pg_dump -Fc -Z$j -d $db_name -U postgres |pigz -$j> $path_to_backup/$db_name.dmp.gz
                s2=$((`date +"%s%3N"` - $st))
                s3=`du -sh $path_to_backup|awk '{print $1}'`
                createdb -U postgres $db_name2
                st=`date +"%s%3N"`
...
do
        for (( i=1; i <= $num_iterations; i++ ))
        do
                s1="directory dump j="$j
                echo $s1"-"$i
                /root/clear.sh
                st=`date +"%s%3N"`
                pg_dump -Fd -j$j -d $db_name -U postgres -f $path_to_backup/$db_name
                s2=$((`date +"%s%3N"` - $st))
        	s3=`du -sh $path_to_backup|awk '{print $1}'`
